...
# Predicting models
modelmodel_mean <- lmer(mean ~ diagnosis + (1 | ID) + (1 | trial) + (1 | study), data_ass3,  REML = F)
modelmodel_var <- lmer(sd ~ diagnosis + (1 | ID) + (1 | trial) + (1 | study), data_ass3, REML = F)

# Get optimal "mean" by predicting data for a matrix for 3 studies
predict(modelmodel_mean, newdata = diag_stud, re.form = ~ (1 | study) , allow.new.levels = T)
predict(modelmodel_var, newdata = diag_stud, re.form = ~ (1 | study), allow.new.levels = T)
...
listoflists <- list(Title = ("Assignment 3"), StudyID = c(49, 50, 51), ArticleID = 47, SAMPLE_SIZE_SZ = c(335, 182, 158), SAMPLE_SIZE_HC = c(352, 184, 128), PITCH_F0_HC_M = c(142.4939, 123.8641, 137.0162), PITCH_F0_HC_SD = c(24.72409, 27.32127, 25.47117), PITCH_F0_SZ_M = c(156.2813, 137.6516, 150.8036), PITCH_F0_SZ_SD = c(24.72409, 27.32127, 25.47117))
